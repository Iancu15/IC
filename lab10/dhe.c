...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "i:p:f:")) != -1) {
        switch (opt) {
            ...
            default:
                fprintf(stderr,
                    "Usage %s [-i SERVER_IP] [-p PORT] [-f RECV_FILENAME]\n",
                    argv[0]);
                exit(EXIT_FAILURE);
        }
    }
    ...
    // TODO 3:get the public key into the BIGNUM buffer pub_key_theirs (what
    // might correspond to BN_bn2bin ?)
    pub_key_theirs = BN_bin2bn(buf_pubkey_theirs, PUB_KEY_LEN, NULL);

    // TODO 4: compute the secret key from our DH structure and the other party
    // public key return the length in the integer n, although we expect it to
    // be PUB_KEY_LEN
