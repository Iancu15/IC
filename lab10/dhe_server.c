...
/**
 * Open file <filename>, read public Diffie-Hellman parameters P and G and store
 * them in <pdhm>
...
int main(int argc, char* argv[]) {
    ...
    pub_key = DH_get0_pub_key(tdh);
    priv_key = DH_get0_priv_key(tdh);
    ...
    while(1) {
        ...
        // TODO 3:get the public key into the BIGNUM buffer pub_key_theirs
        // (what might correspond to BN_bn2bin ?)
        pub_key_theirs = BN_bin2bn(buf_pubkey_theirs, PUB_KEY_LEN, NULL);

        // TODO 4: compute the secret key from our DH structure and the other
        // party public key return the length in the integer n, although we
        // expect it to be PUB_KEY_LEN
        DH_compute_key(buf_secret_key, pub_key_theirs, tdh);

        // Print exchanged secret key
        printf("[server] Exchanged secret key has %d bytes\n", n);
        printf("[server] The exchanged secret key is: ");
        for(k = 0; k < n; k++)
