import numpy as np


# Rijndael S-box
s_box = np.array([])


def hamming_weight(X: np.ndarray) -> np.ndarray:
    """Computes the Hamming weight

    Parameters
    ----------
    X : np.ndarray
        A numpy array or matrix of integer elements.

    Returns
    -------
    np.ndarray
        The Hamming weight for each element from X.
    """
    assert X.dtype in [np.int8, np.int16, np.int32, np.int64], \
        "Expected integer values, but provided %s" % X.dtype
    return np.vectorize(lambda x: bin(x).count("1"))(X)


def convert_mat_to_npy(
    infile: str = '../MATLAB/simdata_small.mat',
    outfile: str = 'simdata_small.npy'
) -> None:
    """Convert .mat (MATLAB) file to .npy (numpy) file. Can be used for 
    ../MATLAB/simdata.mat and ../MATLAB/simdata_small.mat to numpy format. It
    needs scipy to be installed.

    Parameters
    ----------
    infile : str, optional
        The input .mat file, by default '../MATLAB/simdata_small.mat'
    outfile : str, optional
        The output .npy file, by default 'simdata_small.npy'
    """
    import scipy.io
    mat = scipy.io.loadmat(infile)
    np.save(outfile, mat)
