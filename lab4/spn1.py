from utils import *

# Rijndael S-box
sbox = []

# Rijndael Inverted S-box
rsbox = []


def permute4(s):
    """
      Perform a permutatation by shifting all bits 4 positions right.
      The input is assumed to be a 16-bit bitstring
    """
    ps = ''
    ps = ps + s[12:16]
    ps = ps + s[0:12]
    return ps


def permute_inv4(s):
    """
      Perform the inverse of permute4
      The input is assumed to be a 16-bit bitstring
    """
    ps = ''
    ps = ps + s[4:16]
    ps = ps + s[0:4]
    return ps


def spn_1r_reduced_2s(k, x):
    """
      Performs an encryption with a substitution-permutation network.
      Key k = {k1, k2}, total of 16 bits (2 x 8 bits)
      Input x = {x1, x2}, total of 16 bits (2 x 8 bits)
      Both k and x are assumed to be bitstrings.

      Return:
      a 16-bit bitstring containing the encryption y = {y1, y2}
    """

    # Split input and key
    x1 = x[0:8]
    x2 = x[8:16]
    k1 = k[0:8]
    k2 = k[8:16]

    # Apply S-box
    u1 = bitxor(x1, k1)
    v1 = sbox[int(u1, 2)]
    v1 = byte_2_bin(v1)

    u2 = bitxor(x2, k2)
    v2 = sbox[int(u2, 2)]
    v2 = byte_2_bin(v2)

    # Apply permutation
    pin = v1 + v2
    pout = permute4(pin)

    return pout


def spn_1r_full_2s(k, x):
    """
      Performs an encryption with a substitution-permutation network.
      Key k = {k1, k2, k3, k4}, total of 32 bits (4 x 8 bits)
      Input x = {x1, x2}, total of 16 bits (2 x 8 bits)
      Both k and x are assumed to be bitstrings.

      Return:
      a 16-bit bitstring containing the encryption y = {y1, y2}
    """

    # Split input and key
    x1 = x[0:8]
    x2 = x[8:16]
    k1 = k[0:8]
    k2 = k[8:16]
    k3 = k[16:24]
    k4 = k[24:32]

    # Apply S-box
    u1 = bitxor(x1, k1)
    v1 = sbox[int(u1, 2)]
    v1 = byte_2_bin(v1)

    u2 = bitxor(x2, k2)
    v2 = sbox[int(u2, 2)]
    v2 = byte_2_bin(v2)

    # Apply permutation
    pin = v1 + v2
    pout = permute4(pin)

    # Apply final XOR
    po1 = pout[0:8]
    po2 = pout[8:16]
    y1 = bitxor(po1, k3)
    y2 = bitxor(po2, k4)

    return y1 + y2

def reduced_inv_spn(ys, xs):
    y = permute_inv4(ys)
    y1 = y[0:8]
    y2 = y[8:16]
    x1 = xs[0:8]
    x2 = xs[8:16]

    y1_r = byte_2_bin(rsbox[int(y1, 2)])
    y2_r = byte_2_bin(rsbox[int(y2, 2)])
    k1 = bitxor(x1, y1_r)
    k2 = bitxor(x2, y2_r)
    
    return k1 + k2

def reduced_inv_spn_full(xss, yss):
    xss = [str_2_bin(xs) for xs in xss]
    for k3_dec in range(0, 257):
      for k4_dec in range(0, 257):
        k3 = byte_2_bin(k3_dec)
        k4 = byte_2_bin(k4_dec)
        yx = []
        for ys in yss:
          yx.append(bitxor(k3, ys[0:8]) + bitxor(k4, ys[8:16]))

        ks12 = []
        for i in range(0, len(xss)):
          ks12.append(reduced_inv_spn(yx[i], xss[i]))

        are_equal = True
        for k12_i in ks12:
          for k12_j in ks12:
            if k12_i != k12_j:
              are_equal = False
              break
        
        if are_equal:
          return ks12[0] + k3 + k4
          

def main():

    # Run reduced 2-byte SPN
    msg = 'Hi'
    key = '??'  # Find this
    xs = str_2_bin(msg)
    ks = str_2_bin(key)
    ys = spn_1r_reduced_2s(ks, xs)
    print('Two y halves of reduced SPN: ' + ys[0:8] + ' (hex: ' + bin_2_hex(
        ys[0:8]) + '), ' + ys[8:16] + ' (hex: ' + bin_2_hex(ys[8:16]) + ')')
    inv_perm_ys = permute_inv4(ys)
    dec_key = reduced_inv_spn(hex_2_bin('ba52'), xs)
    print(bin_2_str(dec_key))

    # Run full 2-byte SPN
    msg = 'Om'
    key = '????'  # Find this
    xs = str_2_bin(msg)
    ks = str_2_bin(key)
    ys = spn_1r_full_2s(ks, xs)
    print('Two y halves of full SPN (2 bytes): ' + ys[0:8] + ' (hex: ' + bin_2_hex(
        ys[0:8]) + '), ' + ys[8:16] + ' (hex: ' + bin_2_hex(ys[8:16]) + ')')
    dec_key = reduced_inv_spn_full(['Om', 'El', 'an'], [hex_2_bin('0073'), hex_2_bin('d00e'), hex_2_bin('855b')])
    print(bin_2_str(dec_key))


if __name__ == "__main__":
    main()  